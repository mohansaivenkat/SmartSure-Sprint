...
import { PageHeader, Card, StatCard } from '../../shared/components/UI';
...
/* ─────────────────────────────────────────────────────────────
   Donut Chart
 ───────────────────────────────────────────────────────────── */
function DonutChart({ segments, size = 160, stroke = 26 }: any) {
  const r = (size - stroke) / 2;
  const circ = 2 * Math.PI * r;
  const cx = size / 2, cy = size / 2;

  let offset = 0;
  const slices = segments.map((s: any) => {
...
/* ─────────────────────────────────────────────────────────────
 ...
 ───────────────────────────────────────────────────────────── */
function GaugeChart({ value, max = 100, color = '#6366f1', size = 160 }: any) {
  const r = 64; 
...
/* ─────────────────────────────────────────────────────────────
 ...
 ───────────────────────────────────────────────────────────── */
function BarChart({ bars, height = 90 }: any) {
...
export default function AdminReports() {
  ...
  const kpis = [
    { label: 'Total Policies', value: policies,  color: '#6366f1', icon: HiDocumentText },
    { label: 'Total Claims',   value: total,     color: '#06b6d4', icon: HiClipboardList },
      ...
      <div className="rep-root fade-up" style={{ minHeight: '100vh', padding: '32px 24px', maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            {loading && <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-primary)', opacity: .7 }} className="animate-pulse">Loading data...</span>}
            <button className="ref-btn" onClick={() => fetchReports(true)} disabled={refreshing || loading}>
                <HiRefresh style={{ width: 14, height: 14, ...(refreshing || loading ? { animation: 'spin 1s linear infinite' } : {}) }} />
                Refresh
            </button>
          </div>
        </div>
        ...
        {loading ? (
            <div className="py-20 flex flex-col items-center justify-center gap-6">
                <LoadingSpinner />
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14, width: '100%', maxWidth: 1200 }}>
                    {[1, 2, 3, 4, 5].map(i => <div key={i} className="skel" style={{ height: 110 }} />)}
                </div>
            </div>
        ) : error ? (
          ...
          <div className="fade-up">
            ...
            <div className="rep-panel">
              ...
              <div style={{ padding: '18px 22px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
                {[
                  { label: 'Approval Rate', value: approvalRate, suffix: '%', color: '#10b981', desc: 'Of all claims processed', bar: approvalRate },
                  { label: 'Rejection Rate', value: rejectionRate, suffix: '%', color: '#ef4444', desc: 'Claims turned away', bar: rejectionRate },
                  { label: 'Pending Rate', value: total ? +((pending / total) * 100).toFixed(1) : 0, suffix: '%', color: '#f59e0b', desc: 'Awaiting decision', bar: total ? (pending / total) * 100 : 0 },
                  { label: 'Claim Density', value: claimRatio, suffix: '%', color: '#6366f1', desc: 'Claims per policy issued', bar: Math.min(claimRatio, 100) },
                ].map(m => (
