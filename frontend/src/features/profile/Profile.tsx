...
function TiltAvatar({ initial }: { initial: string }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-3xl font-black relative z-10" style={{ textShadow: '0 2px 10px rgba(0,0,0,0.2)' }}>
          {initial}
        </span>
      </div>
      {/* Verified badge */}
...
function StatChip({ icon: Icon, label, value, delay }: StatChipProps) {
  return (
    <motion.div
...
function AnimatedField({ label, icon: Icon, children, index }: AnimatedFieldProps) {
